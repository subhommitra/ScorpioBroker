...
public class RegistryAuthorizationTest {
	...
	/* This method tests registering a CSource with Factory-Admin RBAC role */
	@Test
	@WithMockKeycloakAuth({ "ROLE_Factory-Admin" })
	public void whenUserIsFactoryAdminThenRegisterCSourceAllowed() {
		try {
			when(csourceService.registerCSource(any(), any()))
					.thenReturn(new URI("urn:ngsi-ld:ContextSourceRegistration:csr1a3458"));
			mockMvc.perform(post("/ngsi-ld/v1/csourceRegistrations/").contentType(AppConstants.NGB_APPLICATION_JSONLD)
					.content(payload))
	...
	/* This method tests registering a CSource with Reader RBAC role */
	@Test
	@WithMockKeycloakAuth({ "ROLE_Reader" })
	public void whenUserIsReaderThenRegisterCSourceNotAllowed() {
		try {
			when(csourceService.registerCSource(any(), any()))
					.thenReturn(new URI("urn:ngsi-ld:ContextSourceRegistration:csr1a3458"));
			mockMvc.perform(post("/ngsi-ld/v1/csourceRegistrations/").contentType(AppConstants.NGB_APPLICATION_JSONLD)
	...
	/* This method tests updating a CSource with Factory-Admin RBAC role */
	@Test
	@WithMockKeycloakAuth({ "ROLE_Factory-Admin" })
	public void whenUserIsFactoryAdminThenUpdateCSourceAllowed() {
		try {
			mockMvc.perform(patch("/ngsi-ld/v1/csourceRegistrations/{registrationId}",
	...
	/* This method tests updating a CSource with Reader RBAC role */
	@Test
	@WithMockKeycloakAuth({ "ROLE_Reader" })
	public void whenUserIsReaderThenUpdateCSourceNotAllowed() {
		try {
			mockMvc.perform(patch("/ngsi-ld/v1/csourceRegistrations/{registrationId}",
					"urn:ngsi-ld:ContextSourceRegistration:csr1a3458")
							.contentType(AppConstants.NGB_APPLICATION_JSONLD).content(updatePayload))
	...
	/* This method tests deleting a CSource with Factory-Admin RBAC role */
	@Test
	@WithMockKeycloakAuth({ "ROLE_Factory-Admin" })
	public void whenUserIsFactoryAdminThenDeleteCSourceAllowed() {
		try {
			mockMvc.perform(delete("/ngsi-ld/v1/csourceRegistrations/{registrationId}",
					"urn:ngsi-ld:ContextSourceRegistration:csr1a3458")
	...
	/* This method tests deleting a CSource with Reader RBAC role */
	@Test
	@WithMockKeycloakAuth({ "ROLE_Reader" })
	public void whenUserIsReaderThenDeleteCSourceNotAllowed() {
		try {
			mockMvc.perform(delete("/ngsi-ld/v1/csourceRegistrations/{registrationId}",
					"urn:ngsi-ld:ContextSourceRegistration:csr1a3458")
							.contentType(AppConstants.NGB_APPLICATION_JSONLD))
