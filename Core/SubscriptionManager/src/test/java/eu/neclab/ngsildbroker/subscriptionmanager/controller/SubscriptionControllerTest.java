...
public class SubscriptionControllerTest {
	...
	public void createSubscriptionEntityBadRequestTest() {
		try {
			when(subscriptionService.subscribe(any())).thenThrow(new ResponseException(ErrorType.BadRequestData));
			mockMvc.perform(post("/ngsi-ld/v1/subscriptions").contentType(AppConstants.NGB_APPLICATION_JSON)
					.accept(AppConstants.NGB_APPLICATION_JSONLD).content(subscriptionEntityPayload))
	...
	public void createSubscriptionEntityAlreadyExistTest() {
		try {
			when(subscriptionService.subscribe(any())).thenThrow(new ResponseException(ErrorType.AlreadyExists));
			mockMvc.perform(post("/ngsi-ld/v1/subscriptions").contentType(AppConstants.NGB_APPLICATION_JSON)
					.accept(AppConstants.NGB_APPLICATION_JSONLD).content(subscriptionEntityPayload))
	...
	public void deleteSubscriptionTest() {
		try {
			...
			verify(subscriptionService, times(1)).unsubscribe(any(), ArrayListMultimap.create());
		} catch (Exception e) {
			Assert.fail();
			e.printStackTrace();
		}
	...
	public void updateSubscriptionNotFoundTest() {
		try {
			when(subscriptionService.updateSubscription(any())).thenThrow(new ResponseException(ErrorType.NotFound));
			mockMvc.perform(patch("/ngsi-ld/v1/subscriptions/urn:ngsi-ld:Subscription:211/")
					.contentType(AppConstants.NGB_APPLICATION_JSON).accept(AppConstants.NGB_APPLICATION_JSONLD)
					.content(subscriptionEntityPayload)).andExpect(status().isNotFound());
		} catch (Exception e) {
			Assert.fail();
			e.printStackTrace();
		}
	...
	public void getSubscriptionEntityById() throws Exception {
		when(subscriptionService.getSubscription(any(), any())).thenThrow(new ResponseException(ErrorType.NotFound));
		mockMvc.perform(
